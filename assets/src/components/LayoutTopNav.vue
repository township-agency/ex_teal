<template>
  <header class="fixed flex z-50 h-16 pin-t pin-x">
    <aside class="hidden lg:flex flex-none border-r border-grey bg-white lg:w-1/4 xl:w-1/5 items-center px-4 text-black lg:max-w-sidebar">
      <img
        v-if="config.logo"
        :src="config.logo"
        :alt="config.title">
    ...
    <nav class="flex items-center justify-between flex-grow border-b border-grey bg-grey-lightest px-5">
